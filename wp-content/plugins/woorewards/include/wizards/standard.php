...
class Standard extends \LWS\WOOREWARDS\Wizards\Subwizard
{
			...
			case 'ini':
				return array(
					...
					'help'  => __("Welcome to this Wizard. This tool will help you configure your first loyalty system in less than 5 minutes.", 'woorewards-lite') . "<br/>" .
						__("This wizard is limited for WooRewards Standard.", 'woorewards-lite') . "<br/>" .
						__("If you want to have access to all methods and rewards, please consider trying WooRewards Pro.", 'woorewards-lite'),
					'groups' => array(
						array(
							'fields'  => array(
								array(
			...
			case 'met':
				return array(
					...
					'help'  => __("Your customers will earn points every time they perform the actions defined here.", 'woorewards-lite') . "<br/>" .
						...
						__("Some categories are not accessible in the free version of WooRewards.", 'woorewards-lite'),
					'groups' => array(
						array(
							'fields' => array(
								array(
									'id'    => 'category',
									'title' => __("Select a Category", 'woorewards-lite'),
									'type'  => 'radiogrid', // radiogrid is specific to the wizard
									'extra' => array(
										'source' => array(
											array('value' => 'orders', 'icon' => 'lws-icon lws-icon-shop', 'label' => __("Orders", 'woorewards-lite')),
											array('value' => 'rate', 'icon' => 'lws-icon lws-icon-star-full', 'label' => __("Rate us !", 'woorewards-lite')),
											array('value' => 'product', 'icon' => 'lws-icon lws-icon-barcode', 'class' => 'inactive', 'label' => __("Products", 'woorewards-lite')),
											array('value' => 'social', 'icon' => 'lws-icon lws-icon-b-meeting', 'class' => 'inactive', 'label' => __("Talk about us !", 'woorewards-lite')),
										),
										'default' => 'orders',
									),
								)
	...
	function isValid($step, &$submit)
	{
		...
		if( $step == 'met' )
		{
			...
			$err[] = __("Points for order review expects numeric value greater than zero or leave blank.", 'woorewards-lite');
		}
		else if( $step == 'rew' )
		{
	...
	{
		$e = $pool->getEvents()->filter(function($item)use($type){return $item->getType() == $type;});
		while( $e->count() )
		{
			$item = $e->last();
			$e->remove($item);
	...
	{
		$u = $pool->getUnlockables()->filter(function($item)use($type){return $item->getType() == $type;});
		while( $u->count() )
		{
			$item = $u->last();
			$u->remove($item);
