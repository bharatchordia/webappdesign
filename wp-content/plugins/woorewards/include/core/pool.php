...
class Pool
{
	...
	public function addPoints($userId, $value, $reason='', \LWS\WOOREWARDS\Abstracts\Event $origin=null, $origin2=false)
	{
		...
		$this->getStack($userId)->add($value, $reason, false, $origin?$origin->getId():'', $origin2);
		return $this;
	}
	...
	public function setPoints($userId, $value, $reason='', $origin=null, $origin2=false)
	{
		...
		if( $origin && \method_exists($origin, 'getId') )
			$origin = $origin->getId();
		else
			$origin = '';
		$this->getStack($userId)->set($value, $reason, $origin, $origin2);
		return $this;
	}
	...
	public function usePoints($userId, $value, $reason='', \LWS\WOOREWARDS\Abstracts\Unlockable $origin=null, $origin2=false)
	{
		...
		$this->getStack($userId)->sub($value, $reason, false, $origin?$origin->getId():'', $origin2);
		return $this;
	}
	...
	protected function _payAndContinue($userId, &$unlockable)
	{
		...
		if( $cost > 0 )
			$this->usePoints($userId, $cost, $unlockable->getReason('raw'), $unlockable);
		return ($cost > 0);
	}
