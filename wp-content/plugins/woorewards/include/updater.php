	...
	static function notice($fromVersion, $toVersion)
	{
		...
		if( !(defined('LWS_WOOREWARDS_ACTIVATED') && LWS_WOOREWARDS_ACTIVATED) )
		{
			...
			$a = array(
				...
				'order' => array(
					'name' => _x("Long Watch Studio - WooRewards", 'link to plugin selling site', 'woorewards-lite'),
					'href' => "https://plugins.longwatchstudio.com/product/woorewards/",
				)
			);
			$note = __("<p>You just installed or updated the <b>WooRewards Lite</b> version.</p>", 'woorewards-lite');
			$note .= sprintf(
				__("<p>Do you have a <b>licence key</b> (for Trial or Pro version)? Think about setting it %s and discover all the possibilities of WooRewards.</p>", 'woorewards-lite'),
		...
		if( $wpdb->get_col("SHOW TABLES LIKE '{$icl}'") )
		{
			...
			foreach( $suffixes as $suffix )
			{
				foreach( $strings as $dst => $src )
				{
					$where = array();
					foreach( $src as $trad )
						$where[] = $wpdb->prepare('`name`=%s', sprintf($template, $trad, $suffix));
