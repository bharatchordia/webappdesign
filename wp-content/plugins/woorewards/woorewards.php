...
final class LWS_WooRewards
{
	...
	public function addLicenceLink($links, $file, $data, $status)
	{
		...
		{
			$label = __('Add Licence Key', 'woorewards-lite');
	...
	{
		return __("https://plugins.longwatchstudio.com/docs/woorewards/", 'woorewards-lite');
	}
	...
	{
		\add_filter('lws_adm_license_trial_teaser_texts', function($teaser, $slug){
			if( 'woorewards' == $slug ){
				$teaser = __("This version contains approximatively 5% of the Premium features.", 'woorewards-lite');
				$teaser .= __("WooRewards Pro offers a lot more features than WooRewards Standard.", 'woorewards-lite');
				$teaser .= __("Infinite loyalty systems, sponsorship, badges and achievements are only a few of these features.", 'woorewards-lite');
			}
			return $teaser;
		}, 10, 2);

		\lws_plugin(__FILE__, LWS_WOOREWARDS_PAGE, md5(\get_class() . 'update'), 'LWS_WOOREWARDS_ACTIVATED', 'woorewards.system');
	}
	...
	private function install()
	{
		...
		add_filter( 'image_size_names_choose', function($sizes){
			return array_merge($sizes, array(
				'lws_wr_thumbnail' => __("WooRewards Thumbnail", 'woorewards-lite'),
				'lws_wr_thumbnail_small' => __("WooRewards Thumbnail Small", 'woorewards-lite')
			));
		});
	...
	function testPoolCompletion()
	{
		...
		{
			$content = __("We detect a WooRewards licence downgrade: Click here to restore missing configuration.", 'woorewards-lite');
